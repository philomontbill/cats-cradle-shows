...
- **Mercury East venue detection (Mar 10, 2026):** Both `/boweryballroom` and `/mercurylounge` pages serve identical JavaScript event data (EventData.events.push). Event detail pages have both venue names in site nav. Full-text search for "bowery ballroom" matched on every page. Fix: use `/tm-venue/bowery-ballroom/` and `/tm-venue/mercury-lounge/` pages which list venue-specific events in structured HTML cards.
