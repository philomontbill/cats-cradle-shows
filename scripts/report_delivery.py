...
def ensure_definitions_tab():
    ...
    definitions = [
        ...
        ["Sort Priority", "Numeric sort key (1=most actionable, 9=least). Sorts actionable items to top within each date."],
        [],
        ["Sort Priority Order", ""],
        ["1 — flag", "Marginal match (score 40-69). May be fixable with an override."],
        ["2 — no_results", "YouTube search returned nothing. Worth a manual look."],
        ["3 — api_error", "YouTube API failure. Temporary — may resolve next run."],
        ["4 — code_error", "Bug in scraper code. Needs investigation."],
        ["5 — reused", "Previously searched, keeping null result. Expected no preview."],
        ["6 — skip", "Intentionally not searched (event name, invalid). Expected no preview."],
        ["7 — no_log", "No match log entry. Typically 2nd/3rd opener. Expected no preview."],
        ["8 — verified", "Video assigned (Verified section rows)."],
        ["9 — rejected", "Video failed checks (Rejected section rows)."],
        [],
