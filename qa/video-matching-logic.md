...
### 2. Verifier (scripts/verify_videos.py)
...
### 3. Daily Video Report (generated by verifier)
...
12. **Generic venue image check** (free, no API call) — if the show's image URL matches a known venue placeholder (e.g., Cat's Cradle uses `cradlevenue.png`), flag as likely event rather than band.
13. For each unverified candidate, pull video metadata from YouTube `videos` endpoint (1 quota unit):
    - **View count check** — 5M default cap, 50M for trusted labels/VEVO (see View Count Caps table below)
...
14. Pull channel info from YouTube `channels` endpoint (1 quota unit):
    - **Trusted channel detection** — check if channel is a known label (14 on allowlist) or VEVO channel. Trusted channels bypass channel mismatch and upload age rejections.
...
16. Generate markdown report + CSV covering:
    - **Summary** — counts (verified, rejected, no-preview, changes)
    - **Verified Videos** — what passed and went live
    - **Rejected Candidates** — what failed, with specific reasons
    - **No Preview Queue** — all artists currently showing the popup
17. Post as GitHub Issue (label: `daily-video-report`)
18. Save CSV to `qa/video-report-YYYY-MM-DD.csv`
...
|-----------|-----|-----------|
| Default | 5M | Bands at small venues typically have 10K-2M view videos |
...
4. **Separation of concerns** — scraper finds candidates, verifier validates, report surfaces results. Each can be improved independently.
